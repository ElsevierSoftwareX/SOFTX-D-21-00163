function plots = plotting(Initial_Eta, Initial_u, Data_Projection,...
  ...
  global x x0 Xf t Tf t_res td eta_0 eta0 u_0 u0 
  ...
  if contains(Data_Projection,"on")
   ...
   if contains(L2_Norm,"on")
     Rd_i = (stat_norm_max_i+floor(t_res./Tf));       % index of run down
     Rd_t = Tf*Rd_i/t_res;                            % time of run down
     ...
     subplot(4,1,3);
     plot(x,num(Rd_i,:)), hold on;
     plot(x,ana(Rd_i,:)), hold on;
     plot(x,-td*x), hold off;
     axis([-0.2 6 -0.03 0.04]);
     title(sprintf('Run-down ($t=%0.2f$)',Rd_t), 'Interpreter','latex');
     xlabel('$x$', 'Interpreter','latex');
     ylabel('$\eta$', 'Interpreter','latex');
     legend('Numerical', 'Analytical','Bathymetry');
